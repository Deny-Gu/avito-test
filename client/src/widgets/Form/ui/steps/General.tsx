    ...
    return (
        <> 
            ...
            <Field>
                ...
                <input {...register("description", { required: 'Введите описание объявления' })} type="text" id='description' />
                {errors.description && <FieldError>{errors.description.message?.toString()}</FieldError>}
            </Field>
            <Field>
                <label htmlFor='location'>Локация</label>
                <input {...register("location", { required: 'Введите локацию' })} type="text" id='location' />
                {errors.location && <FieldError>{errors.location.message?.toString()}</FieldError>}
            </Field>
            <Field>
            ...
            <Field>
                ...
                <Controller name="type" control={control} rules={{ required: 'Выберите категорию' }} render={({ field }) => (
                    <Select className="select" placeholder={''} options={selectOptionsCategory} {...field} />
                )} />
                {errors.type && <FieldError>{errors.type.message?.toString()}</FieldError>}
            </Field>
        </>
    )
}